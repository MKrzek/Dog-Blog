...
class MyArticles extends React.Component{
    ...
    showMyArticles=()=>{
        ...
        return _.map(this.props.myArticles, myArticle =>{
            return <MyArticleData myArticle={myArticle} key={myArticle.key} deleteArticle={this.deleteArticle}/>
    ...
    render(){
        return <div>
                    {this.props.myArticles ? (<div>{this.showMyArticles()}</div>) : (<h3>'You have no articles' </h3>)}
               </div>
    }
}
