...
export default class VetDisplay extends React.Component{
    ...
    render(){
        ...
        return <div className='col-md-4 card ml-3 mr-2 mb-4 text-center'>
                    
                    <div className="mt-2">
                    <h3 className='mb-1'>{vet}</h3>
                    <p>{streetName} {streetNumber}</p>
                    <div>{city}</div>
                    <a  target='_blank' href={`https://${www}`} className='mt-1'>{www}</a>
                    <p className='mt-1 mb-3'>{phone}</p>
                    <button className='btn btn-primary mb-3' onClick={this.handleClick}>Show on map</button>
                    </div> 
              </div>
